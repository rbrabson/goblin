...
build-windows:
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/windows/amd64/dgame cmd/dgame/main.go  # windows
...
build-linux:
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/linux/amd64/dgame cmd/dgame/main.go  # linux
...
build-mac-amd:
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/macos/amd64/dgame cmd/dgame/main.go  # mac osx intel chip
...
build-mac-arm:
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/macos/arm64/dgame cmd/dgame/main.go  # mac osx arm chip
