...
func (h *Heist) Start() (*HeistResult, error) {
	...
	goodResults := h.theme.Good
	badResults := h.theme.Bad
	...
	log.WithFields(log.Fields{"Escaped": len(results.Escaped), "Apprehended": len(results.Apprehended), "Dead": len(results.Dead)}).Debug("heist results")
