...
func GetTheme(g *guild.Guild) *Theme {
	...
	config := GetConfig(g)
	themes, err := readThemes(g, config.Theme)
	if err != nil {
		log.WithFields(log.Fields{"guild": g.GuildID, "error": err}).Error("unable to read themes")
	}

	// Find and return the configured theme for the guild
	for _, theme := range themes {
		if theme.Name == config.Theme {
			log.WithFields(log.Fields{"guild": g.GuildID, "theme": theme.Name}).Debug("found theme")
			return theme
		}
	}

	// No matching theme was found, so just use any theme that is available
	if len(themes) > 0 {
		theme := themes[0]
		log.WithFields(log.Fields{"guild": g.GuildID, "wanted": config.Theme, "using": theme.Name}).Debug("matching theme not found, using any theme")
		return theme
	}

	// No themes were found in the DB, so create the default theme and use that
...
func (theme *Theme) String() string {
	return fmt.Sprintf("Theme{ID=%s, GuildID=%s, ThemeID=%s, Good=%d, Bad=%d, Jail=%s, OOB=%s, Police=%s, Bail=%s, Crew=%s, Sentence=%s, Heist=%s, Vault=%s}",
		theme.ID,
		theme.GuildID,
		theme.Name,
		len(theme.Good),
